...
if which gradlew 2>&1 > /dev/null; then
    ...
    cp -f "$BUILD_LIB/outputs/aar/utilsLib-$BUILD_TYPE.aar" "$BUILD_PATH/projects" || exit 1
...
EOF
        ...
        cp "$BUILD_PATH/projects/utilsLib-$BUILD_TYPE.aar" "$BUILD_BINTRAY/library.aar" || exit 1

        btPackage="utils-lib"
        btGroupId="com.spazedog.lib"
        btVersion="$(grep 'versionName' "$BUILD_PATH/projects/utilsLib/build.gradle" | sed 's/.*\"\(.*\)\"/\1/')"
