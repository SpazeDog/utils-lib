...
BUILD_SRC="$(readlink -f "$(dirname $0)")"
BUILD_NAME=$(basename "$BUILD_SRC")

LIB_DST=~/.gradle/build/$BUILD_NAME/utilsLib
LIB_SRC="$BUILD_SRC/src"
LIB_PROJECT="$BUILD_SRC/projects/utilsLib"
LIB_FILE=$LIB_DST/outputs/aar/utilsLib-$BUILD_TYPE.aar
LIB_PKG="$LIB_PROJECT/../utilsLib-$BUILD_TYPE.aar"

BINTRAY_DST=~/.gradle/build/$BUILD_NAME/bintray
BINTRAY_PACKAGE="utils-lib"
